...
# Force unbuffered output so Railway logs work
...
MY_CHAT_ID_RAW = os.environ.get('CHAT_ID', '')

print(f"TELEGRAM_TOKEN set: {bool(TELEGRAM_TOKEN)}", flush=True)
print(f"CHAT_ID set: {bool(MY_CHAT_ID_RAW)}", flush=True)

if not TELEGRAM_TOKEN:
    print("ERROR: TELEGRAM_TOKEN not set!", flush=True)
    sys.exit(1)

if not MY_CHAT_ID_RAW:
    print("ERROR: CHAT_ID not set!", flush=True)
    sys.exit(1)

try:
    MY_CHAT_ID = int(MY_CHAT_ID_RAW)
except ValueError:
    print(f"ERROR: CHAT_ID is not a valid integer: {MY_CHAT_ID_RAW}", flush=True)
    sys.exit(1)
...
bot_running      = True
pair_cache       = {}
bot_start_time   = time.time()  # ignore messages older than this
...
def send_message(text):
    ...
    try:
        ...
        if not r.json().get("ok"):
            print(f"Telegram failed: {r.json()}", flush=True)
...
def find_kraken_pair(symbol):
    ...
    try:
        ...
        print(f"Pair lookup error: {e}", flush=True)
        return None
...
def get_ohlcv_4h(pair):
    ...
    try:
        ...
        print(f"OHLCV error {pair}: {e}", flush=True)
        return []
        ...
        try:
            ...
            for u in updates:
                ...
                if msg.get("chat", {}).get("id") != MY_CHAT_ID:
                    continue
                # Skip messages sent before bot started (avoids replaying old /stop etc.)
                msg_time = msg.get("date", 0)
                if msg_time < bot_start_time:
                    print(f"⏭ Skipping old message: {msg.get('text', '')}", flush=True)
                    continue
                text = msg.get("text", "")
                if text.startswith("/"):
...
def check_fib(pair, sym):
    try:
        ...
        if not price:
            return
        print(f"{sym} | {fmt(price)} | H:{fmt(high)} L:{fmt(low)}", flush=True)
...
# ------------------ MAIN ------------------
def main():
    print(f"🤖 Bot starting | Chat ID: {MY_CHAT_ID}", flush=True)
    send_message(
    ...
    while bot_running:
        try:
            if monitored_coins:
                print(f"\n[{time.strftime('%H:%M:%S')}] Checking {list(monitored_coins.values())}", flush=True)
                for pair, sym in list(monitored_coins.items()):
                    check_fib(pair, sym)
                    time.sleep(2)
            else:
                print(f"[{time.strftime('%H:%M:%S')}] Waiting — send /add SOL to bot", flush=True)
